...
class Plotter:
    ...
    def plot(self, point_data=None, cell_data=None, info=None):
        ...
        fig, axs = OLD.simfempy.meshes.plotmesh.meshWithData(self.heat.mesh, point_data=point_data, cell_data=cell_data, addplots=addplots)
        plt.show()

#----------------------------------------------------------------#
class Heat(OLD.simfempy.applications.heat.Heat):
    ...
    def computeDResAdjW(self, param, u):
        ...
        for j in range(self.nmeasures):
            for k in range(self.nmeasures):
                if k==j: self.problemdata.rhspoint[pointids[k]] = OLD.simfempy.solvers.optimize.RhsParam(1)
    ...
    def computeAdj(self, param, r, u, z):
        ...
        self.problemdata.rhspoint = {}
        for j in range(self.nmeasures):
            self.problemdata.rhspoint[pointids[j]] = OLD.simfempy.solvers.optimize.RhsParam(r[j])
        self.kheatcell = self.kheat(self.mesh.cell_labels)
...
def compute_j(diffglobal):
    ...
    OLD.simfempy.meshes.plotmesh.plotmesh(mesh, ax=axs[0, 2], title='Mesh with measures')
...
def testholes(diffglobal, nholes):
    ...
    optimizer = OLD.simfempy.solvers.optimize.Optimizer(heat, nparam=nholes, nmeasure=nmeasurepoints, regularize=regularize, param0=diffglobal * np.ones(nholes))
