...
class Plotter:
    ...
    def plot(self, point_data=None, cell_data=None, info=None, title=""):
        ...
        if info is None:
            ...
            self.info = info
            addplots = None
        fig, axs = OLD.simfempy.meshes.plotmesh.meshWithData(self.heat.mesh, point_data=point_data, cell_data=cell_data, addplots=addplots, title=title)
        plt.show()

#----------------------------------------------------------------#
class EIT(OLD.simfempy.applications.heat.Heat):
...
def test():
    ...
    optimizer = OLD.simfempy.solvers.optimize.Optimizer(eit, nparam=nparams, nmeasure=nmeasures, regularize=regularize, param0=diffglobal * np.ones(nparams))
