...
def createMesh():
    ...
    holes.append([[-0.5, -0.75], [-0.5, -1.25], [0.5, -1.25], [0.5, -0.75]])
    geometry = OLD.simfempy.meshes.geomdefs.unitsquareholes.Unitsquareholes(rect=(-1.1, 1.1, -2, 2), holes=holes, h=0.2)
    mesh = pygmsh.generate_mesh(geometry)
    return OLD.simfempy.meshes.simplexmesh.SimplexMesh(mesh=mesh)
...
def test(mesh, problemdata):
    fem = 'p1' # or fem = 'cr1
    heat = OLD.simfempy.applications.heat.Heat(problemdata=problemdata, fem=fem, plotk=True)
    heat.setMesh(mesh)
    point_data, cell_data, info = heat.solve()
    print(f"fem={fem} {info['timer']}")
    print(f"postproc: {info['postproc']}")
    OLD.simfempy.meshes.plotmesh.meshWithData(mesh, point_data=point_data, cell_data=cell_data, title=fem)
    plt.show()
