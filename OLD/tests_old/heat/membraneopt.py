...
#----------------------------------------------------------------#
class Membrane(OLD.simfempy.applications.heat.Heat):
    def __init__(self, **kwargs):
        ...
        for label in self.param_labels:
            self.problemdata.rhscell[label] = OLD.simfempy.solvers.optimize.RhsParam(self.param[self.param_labels_inv[label]])
    ...
    def plot(self, **kwargs):
        ...
        fig, axs = OLD.simfempy.meshes.plotmesh.meshWithData(self.mesh, point_data=point_data, cell_data=cell_data)
        plt.show()
    ...
    def computeDRes(self, param, u, du):
        ...
        for i in range(self.nparam):
            for label in self.param_labels:
                self.problemdata.rhscell[label] = OLD.simfempy.solvers.optimize.RhsParam(0)
            self.problemdata.rhscell[self.param_labels[i]] = OLD.simfempy.solvers.optimize.RhsParam(1)
    ...
    def computeDResAdjW(self, param, u):
        ...
        for j in range(self.nmeasures):
            for k in range(self.nmeasures):
                if k==j: self.problemdata.rhscell[self.measure_labels[k]] = OLD.simfempy.solvers.optimize.RhsParam(1)
    ...
    def computeAdj(self, param, r, u, z):
        ...
        self.problemdata.rhscell = {}
        for j in range(self.nmeasures):
            self.problemdata.rhscell[self.measure_labels[j]] = OLD.simfempy.solvers.optimize.RhsParam(r[j])
...
def test():
    ...
    plt.show()
    problemdata = OLD.simfempy.applications.problemdata.ProblemData(bdrycond=bdrycond)
    ...
    optimizer = OLD.simfempy.solvers.optimize.Optimizer(membrane, nparam=nparams, nmeasure=nmeasures, regularize=regularize, param0=np.zeros(nparams))
