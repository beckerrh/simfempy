...
class Plotter:
    ...
    def plot(self, **kwargs):
        ...
        fig, axs = OLD.simfempy.meshes.plotmesh.meshWithData(self.solver.mesh, **kwargs)
        plt.show()

#----------------------------------------------------------------#
class EIT(OLD.simfempy.applications.laplacemixed.LaplaceMixed):
    ...
    def conductivityinv(self, label):
        ...
        return self.diffglobalinv*np.exp(self.scale*self.param[self.param_labels_inv[label]])
    ...
    def _diffinv2param(self, diffinv):
        return np.log(diffinv/self.diffglobalinv)/self.scale

    def _param2x(self, param):
        return self.diffglobalinv*np.exp(self.scale*param)

    def computeAis(self, param):
        ...
        for i in range(self.nparam):
            ...
            self.dcoeff = self.scale*self.diffglobalinv*np.exp(param[i])
