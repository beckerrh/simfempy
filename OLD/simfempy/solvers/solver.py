...
import OLD.simfempy.tools.timer
import OLD.simfempy.tools.iterationcounter
import OLD.simfempy.applications.problemdata
...
class Solver(object):
    ...
    def defineAnalyticalSolution(self, exactsolution, random=True):
        ...
        return OLD.simfempy.tools.analyticalsolution.analyticalSolution(exactsolution, dim, self.ncomp, random)
    ...
    def __init__(self, **kwargs):
        ...
        self.timer = OLD.simfempy.tools.timer.Timer(verbose=0)
        if 'geometry' in kwargs:
            geometry = kwargs.pop('geometry')
            self.mesh = OLD.simfempy.meshes.simplexmesh.SimplexMesh(geometry=geometry, hmean=1)
