...
#=================================================================#
class LaplaceMixed(OLD.simfempy.solvers.solver.Solver):
    """
    """
    ...
    def __init__(self, **kwargs):
        ...
        if 'fem' in kwargs:
            fem = kwargs.pop('fem')
            if fem =='bv0':
                self.femv = OLD.simfempy.fems.fembv0.FemBV0()
            else:
                self.femv = OLD.simfempy.fems.femrt0.FemRT0(massproj=massproj)
        else:
            self.femv = OLD.simfempy.fems.femrt0.FemRT0(massproj=massproj)
        if 'diff' in kwargs:
