...
def rectangle():
    ...
    holes.append([[-0.5, -0.75], [-0.5, -1.25], [0.5, -1.25], [0.5, -0.75]])
    geom = OLD.simfempy.meshes.geomdefs.unitsquareholes.Unitsquareholes(rect=(-1.1, 1.1, -2, 2), holes=holes, h=h)
...
heat = OLD.simfempy.applications.heat.Heat(problemdata=data, mesh=mesh)
point_data, cell_data, info = heat.solveLinearProblem()
print(f"{info['timer']}")
print(f"{info['iter']}")
print(f"postproc: {info['postproc']}")
OLD.simfempy.meshes.plotmesh.meshWithData(mesh, point_data=point_data, cell_data=cell_data)
plt.show()