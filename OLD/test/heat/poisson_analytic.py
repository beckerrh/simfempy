...
def getGeometryAndData(geomname = "unitcube"):
    ...
    if geomname == "unitline":
        ...
        geometry = OLD.simfempy.meshes.geomdefs.unitline.Unitline()
...
def test_analytic(exactsolution="Linear", geomname = "unitsquare", verbose=1, fems=['p1'],methods=['trad']):
    ...
    comp = OLD.simfempy.tools.comparemethods.CompareMethods(sims, verbose=verbose)
...
def test_solvers(geomname='unitcube', fem = 'p1', method='trad'):
    ...
    mesh = OLD.simfempy.meshes.simplexmesh.SimplexMesh(geometry=geometry, hmean=hmean)
    heat.setMesh(mesh)
    print(f"N = {mesh.ncells} heat.linearsolvers={heat.linearsolvers}")
    A = heat.matrix()
    b,u = heat.computeRhs()
    timer = OLD.simfempy.tools.timer.Timer(name=fem + '_' + geomname + '_' + str(mesh.ncells))
