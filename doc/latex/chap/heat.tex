...
\begin{aligned}
\vec{q} = - \kdiff \nabla T\\
\rho C_p \dt{T} + \div\left(\vec{v} T\right)+\div\vec{q} = \dot{q} \quad \mbox{on $\Omega$}\\
T = \TD \quad \mbox{in $\GammaD$}\\
\kdiff\frac{\partial T}{\partial n} = \qN \quad \mbox{on $\GammaN$}\\
c_R T + \kdiff\frac{\partial T}{\partial n}= \qR \quad \mbox{on $\GammaR$}
\end{aligned}
\right.
%
\end{equation}
...
Let $H^1_{\phi}:= \SetDef{T\in H^1(\Omega)}{\Rest{T}{\GammaD} = \phi}$. The standard weak formulation looks for 
